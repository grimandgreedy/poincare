...
struct PersistedPlotEntryWire {
    ...
    relationship: PersistedPlotRelationship,
    #[serde(flatten)]
    content: PersistedPlotEntryContent,
}

#[derive(Deserialize)]
#[serde(untagged)]
enum PersistedPlotEntryContent {
    Current {
        name: String,
        visible: bool,
        domain: Domain,
...
impl<'de> Deserialize<'de> for PersistedPlotEntry {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let wire = PersistedPlotEntryWire::deserialize(deserializer)?;
        let spec = match wire.content {
            PersistedPlotEntryContent::Current {
                name,
                visible,
                domain,
                resolution,
            ...
            PersistedPlotEntryContent::Legacy {
                name,
                visible,
                domain,
                resolution,
                ...
                name,
                visible,
                domain: domain.to_domain(),
                resolution: resolution.to_resolution(),
                style: style.to_plot_style(),
                definition: kind.to_plot_kind(),
            },
        };
        Ok(Self {
            plot_id: wire.plot_id,
            parent_plot_id: wire.parent_plot_id,
...
impl DocumentSnapshot {
    ...
    pub(crate) fn into_document(self) -> Document {
        ...
        doc.plots = restore_plot_entries(graph.as_ref(), &plots);
        doc.axis_config = graph
            .map(|graph| graph.axis_config)
            .unwrap_or_else(|| axis_config.to_axis_config());
    ...
    fn apply_to_app(&self, app: &mut App) {
        ...
        doc.plots = restore_plot_entries(self.graph.as_ref(), &self.plots);
        doc.axis_config = self
            .graph
            .as_ref()
            .map(|graph| graph.axis_config.clone())
            .unwrap_or_else(|| self.axis_config.to_axis_config());
        doc.normalize_plot_hierarchy();
...
fn restore_plot_entries(graph: Option<&GraphSpec>, plots: &[PersistedPlotEntry]) -> Vec<PlotEntry> {
    if !plots.is_empty() {
        return plots
            .iter()
...
impl PersistedPlotEntry {
    fn to_plot_entry(&self) -> PlotEntry {
        let mut entry = PlotEntry::from_plot_spec(self.spec.clone());
        entry.plot_id = self.plot_id;
        entry.parent_plot_id = self.parent_plot_id;
        entry.relationship = self.relationship.to_plot_relationship();
        entry
    }
...
mod tests {
    ...
    #[test]
    fn document_snapshot_writes_lib_plot_definition() {
        let snapshot = DocumentSnapshot::from_document(&sample_document());
        let json = serde_json::to_value(&snapshot).expect("serialize snapshot");
        let plot = &json["plots"][0];

        assert!(plot.get("definition").is_some());
        assert!(plot.get("kind").is_none());
        assert!(json["graph"].get("plots").is_some());
    }

    #[test]
    fn legacy_plot_kind_entries_still_load() {
        let json = r#"
        {
          "plots": [
            {
              ...
              "name": "legacy wave",
              "visible": true,
              "domain": {"x": [-5.0, 5.0], "y": [-5.0, 5.0], "z": [-5.0, 5.0]},
              "resolution": {"u": 64, "v": 48},
              "style": {
                "colour_mode": {"Solid": [1.0, 0.0, 0.0, 1.0]},
                "opacity": 1.0,
                "two_sided": true,
                "line_width": 2.0,
                "point_size": 6.0,
              ...
              "kind": {"ExprCartesian": {"expression": "sin(x)", "parameters": []}}
            }
          ]
        }
        "#;

        let snapshot: DocumentSnapshot = serde_json::from_str(json).expect("legacy snapshot");
        let doc = snapshot.into_document();

        assert_eq!(doc.plots[0].plot_id, 7);
        assert_eq!(doc.plots[0].name, "legacy wave");
        match &doc.plots[0].kind {
            PlotKind::ExprCartesian { expression, .. } => assert_eq!(expression, "sin(x)"),
            _ => panic!("expected legacy Cartesian expression plot"),
        }
    }
